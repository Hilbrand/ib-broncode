 ...
 * @returns
 */
function algemeneHeffingsKorting(toetsingsinkomen, maxBelasting, aow, details = false) {
  let ahkt = aow ? AHK.AOW : AHK.V;
  for (let ahk of ahkt) {
    if (toetsingsinkomen < ahk.inkomen.tot) {
      let algemeneHeffingsKorting = ahk.minimum - (toetsingsinkomen - ahk.minus) * ahk.factor;
      let ahkBerekend = Math.round(Math.min(maxBelasting, algemeneHeffingsKorting));
 ...
 * @returns
 */
function arbeidskorting(arbeidsinkomen, maxArbeidsinkomen, aow, details = false) {
...
function maxArbeidsKorting(toetsingsInkomen, maxBelasting, aow) {
  return inkomstenBelasting(toetsingsInkomen, aow) - algemeneHeffingsKorting(toetsingsInkomen, maxBelasting, aow);
}
