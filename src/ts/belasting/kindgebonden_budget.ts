...
import data from "../../js/belasting/belasting_data.js";
...
function rekenBasis(jaar: string, aantalKinderen: number, toeslagenPartner: boolean): number {
  ...
  return tabelj.Kind1 + +((aantalKinderen - 1) * tabelj.kindVolgend) + (toeslagenPartner ? 0 : tabelj.VHgeenTP);
}
...
function kindgebondenBudget(
  ...
  if (maxKindergebondenBudget > 0) {
    ...
    if (toetsinkomen > maxInk) {
      kgb = Math.max(0, maxKindergebondenBudget - Math.floor((toetsinkomen - maxInk) * (tabelj.TslgTP / 100)));
    } else {
      kgb = maxKindergebondenBudget;
    }
    return tabelj?.afhInk && toetsinkomen > tabelj?.afhInk
      ? Math.round(Math.max(0, kgb * (tabelj?.afbouwfactor || 1)))
      : kgb;
  } else {
    return 0;
  }
