...
import { isVolwassene } from "../functies";
import { InkomenType, PersoonType } from "../types";
...
function hypotheekrenteaftrekVanaf2025(
  jaar: string,
  inkomen: number,
  ...
  if (hypotheekRenteAftrek > 0) {
    for (let schijf of ibTabel) {
      if ((!schijf.vanaf || inkomen >= schijf.vanaf) && (!schijf.tot || inkomen < schijf.tot)) {
        return Math.ceil(
          hypotheekRenteAftrek * (schijf.percentage > ibTabel[1].percentage ? ibTabel[1].percentage : schijf.percentage)
        );
      }
    }
  }
