...
import { heeftInkomen } from "../functies";
import { tekstVerdiener } from "../samenvatting";
...
function brutoInkomen(titelAanvulling: string): TableBaseColumn {
  return {
    ...
    key: "brutoloon",
    align: "right",
    render: (_) => Math.round(+_.brutoloon1).toLocaleString(),
  };
}
...
const JV_HUUR_KOLOMMEN: DataTableColumns<BeschikbaarInkomenResultaatType> = [
  PP_KOLOM,
  IBBOX1_KOLOM,
  AHK_KOLOM,
  AK_KOLOM,
...
const BD_KOLOMMEN: DataTableColumns<BelastingDrukResultaatType> = [AI_KOLOM, IBBOX1_KOLOM, BD_KOLOM];
...
export function vjHuurKolommen(personen: PersoonType[]): DataTableColumns<BeschikbaarInkomenResultaatType> {
  const kolommen = [];

  kolommen.push(brutoInkomen(tekstVerdiener(undefined, personen, "-")));
  kolommen.push(...JV_HUUR_KOLOMMEN);
  return kolommen;
}
