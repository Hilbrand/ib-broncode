...
import kgb from "../../../src/ts/belasting/kindgebonden_budget";
...
test("test Kindgebonden Budget 40.000, alleenstaand, 1 kind 12-", () => {
  expect(kgb.kindgebondenBudget(JAAR, 40000, kgb.maxKindgebondenBudget(JAAR, A1K, false), false)).toEqual(
    A1K_MAX - 1007
  );
});
...
test("test Kindgebonden Budget 40.000, alleenstaand, 1 kind 12-, 1 kind 12-15, 1 kind 16-17", () => {
  expect(kgb.kindgebondenBudget(JAAR, 40000, kgb.maxKindgebondenBudget(JAAR, A3K, false), false)).toEqual(
    A3K_MAX - 1007
  );
});
